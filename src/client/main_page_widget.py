...
class MainPageMenu(AnimatedPanel):
    ...
    def load_music(self, my_music_flag: bool=False) -> None:
        files: list = get_music_in_music_dir(True, my_music_flag)
    ...
    class MusicFrame(QtWidgets.QFrame):
        ...
        def set_audio(self) -> None:
            if self.main_win.music_session.current_music != self.music:
                self.toggle_pressed()
                self.main_win.music_info_widget.play_button.setEnabled(True)
                self.main_win.music_info_widget.like_button.setEnabled(True)

                if self.main_win.music_session.widget:
                    if self.main_win.music_session.widget.toggle:
                        self.main_win.music_session.widget.toggle_pressed()
                self.stop()
                time.sleep(0.1)
